...
public:
	...
	//vaylb added
    virtual String8 getConsumerName();
	virtual void setVideoHookFlag(int flag);
	virtual void setSlaveIp(String16 ip);
	virtual void setScreenSplit(bool split);
	virtual void setSlaveIpNum(int num);
...
private:
	...
	//vaylb added for video hook
public:
	//status_t buffer_mirror(BufferItem* item_dest,const BufferItem *item_src,int buf,int64_t timestamp,uint64_t framenum);
	//status_t initVideoShare(String16 slave_ip);
	//status_t setUpSlaveIp(String16 ip);
	status_t 						getVideoShare();
	int								getCanWriteID();
	void							resetSendFlag();
	void							clip_resize(unsigned char* dst, unsigned char* src, int width, int height);

	bool 							video_hook_flag;
	//BufferItem video_hook_flash;
	
	Vector<sp<GraphicBuffer> > 		video_graphic_pool;
	int 							video_hook_index;
	String16 						video_slave_ip;
	sp<IVideoShare> 				video_share_client;
	sp<IMemory>						video_share_memory;
	CblkVideo*						video_share_cblk;
	void*							video_share_addr;
	sp<nv12rescale>					video_nv12_rescale;
	bool							video_screen_split;

	//added for middleware
	int 							video_slave_num;
	bool							video_middleware_flag; //set true when use for middleware
