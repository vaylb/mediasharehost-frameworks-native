...
String8 MonitoredProducer::getConsumerName(){
	return mProducer->getConsumerName();
}

void MonitoredProducer::setVideoHookFlag(int flag){
	mProducer->setVideoHookFlag(flag);
}

void MonitoredProducer::setSlaveIp(String16 ip){
	mProducer->setSlaveIp(ip);
}

void MonitoredProducer::setScreenSplit(bool split){
	mProducer->setScreenSplit(split);
}

void MonitoredProducer::setSlaveIpNum(int num){
	mProducer->setSlaveIpNum(num);
}
