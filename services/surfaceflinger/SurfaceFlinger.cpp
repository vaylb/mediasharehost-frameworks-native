...
//vaylb: set_video_hook
status_t SurfaceFlinger::setVideoHook(bool flag) {

    ALOGE("vaylb-->SurfaceFlinger::setVideoHook = %d",flag);
	video_hook_flag = flag;
	setVideoHook_l(flag);

    return NO_ERROR;
}

void SurfaceFlinger::setVideoHook_l(bool flag) {
	const LayerVector& currentLayers(mCurrentState.layersSortedByZ);
    size_t count = currentLayers.size();
	int num = 0;
    for (size_t i=0 ; i<count ; i++) {
        const sp<Layer>& layer(currentLayers[i]);
		sp<IGraphicBufferProducer> bufferProducer = layer->mProducer;
		if (strcmp(bufferProducer->getConsumerName(),"SurfaceView")==0){
			bufferProducer->setVideoHookFlag(flag);
			num++;
		}
    }
	ALOGE("vaylb-->setVideoHook_l set %d Layer's video_hook_flag to %d",num,flag);
}

status_t SurfaceFlinger::initVideoShareClient() {

    ALOGE("vaylb-->SurfaceFlinger::initVideoShareClient");
	video_share_client_flag = true;
	//initVideoShareClient_l();
    return NO_ERROR;
}

status_t SurfaceFlinger::setSlaveIp(String16 ip) {

    ALOGE("vaylb-->SurfaceFlinger::setSlaveIp");
	video_slave_ip = ip;
	setSlaveIp_l(ip);
...
void SurfaceFlinger::setSlaveIp_l(String16 ip) {
	const LayerVector& currentLayers(mCurrentState.layersSortedByZ);
    size_t count = currentLayers.size();
    ...
    for (size_t i=0 ; i<count ; i++) {
		...
		sp<IGraphicBufferProducer> bufferProducer = layer->mProducer;
		if (strcmp(bufferProducer->getConsumerName(),"SurfaceView")==0){
			bufferProducer->setSlaveIp(ip);
		}
    }
}

status_t SurfaceFlinger::setScreenSplit(bool split) {

	ALOGE("vaylb-->SurfaceFlinger::setScreenSplit to %d",split);
	video_screen_split = split;
	setScreenSplit_l(video_screen_split);
    return NO_ERROR;	
}

void SurfaceFlinger::setScreenSplit_l(bool split) {
	const LayerVector& currentLayers(mCurrentState.layersSortedByZ);
    size_t count = currentLayers.size();
    ...
    for (size_t i=0 ; i<count ; i++) {
		...
		sp<IGraphicBufferProducer> bufferProducer = layer->mProducer;
		if (strcmp(bufferProducer->getConsumerName(),"SurfaceView")==0){
			bufferProducer->setScreenSplit(split);
		}
    }
}

status_t SurfaceFlinger::setSlaveNum(int num) {

    ALOGE("vaylb-->SurfaceFlinger::setSlaveNum:%d",num);
	video_middleware = true;
	video_slave_num = num;
	setSlaveNum_l(num);
...
void SurfaceFlinger::setSlaveNum_l(int num) {
	const LayerVector& currentLayers(mCurrentState.layersSortedByZ);
    size_t count = currentLayers.size();
	int cou = 0;
    for (size_t i=0 ; i<count ; i++) {
        const sp<Layer>& layer(currentLayers[i]);
		sp<IGraphicBufferProducer> bufferProducer = layer->mProducer;
		ALOGE("vaylb-->layer name = %s",layer->mName.string());
		if (strcmp(layer->mName.string(),"SurfaceView")==0){
			bufferProducer->setSlaveIpNum(num);
			cou++;
		}
    }
	ALOGE("vaylb-->setSlaveNum_l set mCurrentState %d Layer's slavenum to %d",cou,num);
}
...
{
    //ALOGE("vaylb-->createLayer for (%d x %d), name=%s", w, h, name.string());
    if (int32_t(w|h) < 0) {
	...
	if(video_hook_flag)
	{	
		ALOGE("vaylb-->Create Layer setVideoHook to %d",video_hook_flag);
		//modify for middle
		if(video_middleware)setSlaveNum_l(video_slave_num);
		else setSlaveIp_l(video_slave_ip);
		setScreenSplit_l(video_screen_split);
		setVideoHook_l(video_hook_flag);
		//initVideoShareClient_l();
	}
    return result;
}
