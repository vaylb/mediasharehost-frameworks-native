...
public:
    ...
    virtual String8 getConsumerName(){
        Parcel data, reply;
        data.writeInterfaceToken(IGraphicBufferProducer::getInterfaceDescriptor());
        remote()->transact(GET_CONSUMER_NAMES, data, &reply);
	...
	virtual void setVideoHookFlag(int flag){
		...
		remote()->transact(SET_VIDEO_HOOK_FLAG, data, &reply);
	}
	...
	virtual void setSlaveIp(String16 ip){
		...
		remote()->transact(SET_SLAVE_IP, data, &reply);
	}
	...
	virtual void setScreenSplit(bool split)
    {
        ...
        remote()->transact(SET_SCREEN_SPLIT, data, &reply);
    }
	...
	virtual void setSlaveIpNum(int num)
    {
        ...
        data.writeInterfaceToken(IGraphicBufferProducer::getInterfaceDescriptor());
		data.writeInt32(num);
        remote()->transact(SET_SLAVE_NUM, data, &reply);
    }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case GET_CONSUMER_NAMES:{
            CHECK_INTERFACE(IGraphicBufferProducer, data, reply);
            String8 name = getConsumerName();
            reply->writeString8(name);
            return NO_ERROR;
        }break;
		case SET_VIDEO_HOOK_FLAG:{
			CHECK_INTERFACE(IGraphicBufferProducer, data, reply);
            int flag = data.readInt32();
			setVideoHookFlag(flag);
			return NO_ERROR;
		}break;
		case SET_SLAVE_IP: {
            CHECK_INTERFACE(IGraphicBufferProducer, data, reply);
            setSlaveIp(data.readString16());
            return NO_ERROR;
        }break;
