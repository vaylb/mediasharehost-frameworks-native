...
public:
	...
	virtual status_t setVideoHook(bool flag)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
		data.writeInt32(flag ? 1 : 0);
        remote()->transact(BnSurfaceComposer::SET_VIDEO_HOOK, data, &reply);
        return reply.readInt32();
    }

	virtual status_t initVideoShareClient()
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
        remote()->transact(BnSurfaceComposer::INIT_VIDEOSHARE_CLIENT, data, &reply);
        return reply.readInt32();
    }

	virtual status_t setSlaveIp(String16 ip)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
		data.writeString16(ip);
        remote()->transact(BnSurfaceComposer::SET_SLAVE_IP, data, &reply);
        return reply.readInt32();
    }

	virtual status_t setScreenSplit(bool split)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
		data.writeInt32(split ? 1 : 0);
        remote()->transact(BnSurfaceComposer::SET_SCREEN_SPLIT, data, &reply);
        return reply.readInt32();
    }

	virtual status_t setSlaveNum(int num)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
		data.writeInt32(num);
        remote()->transact(BnSurfaceComposer::SET_SLAVE_NUM, data, &reply);
        return reply.readInt32();
    }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
		...
		case SET_VIDEO_HOOK: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
			bool flag = bool(data.readInt32());
            status_t result = setVideoHook(flag);
            reply->writeInt32(result);
            return NO_ERROR;
        }break;
		case INIT_VIDEOSHARE_CLIENT: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            status_t result = initVideoShareClient();
            reply->writeInt32(result);
            return NO_ERROR;
        }break;
		case SET_SLAVE_IP: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            status_t result = setSlaveIp(data.readString16());
            reply->writeInt32(result);
            return NO_ERROR;
        }break;
		case SET_SCREEN_SPLIT: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            setScreenSplit(bool(data.readInt32()));
            return NO_ERROR;
        }break;
		case SET_SLAVE_NUM:{
			CHECK_INTERFACE(ISurfaceComposer, data, reply);
            setSlaveNum(data.readInt32());
            return NO_ERROR;
		}break;
        default: {
