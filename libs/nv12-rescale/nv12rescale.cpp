...
nv12rescale::nv12rescale()
{
	ALOGE("vaylb-->NV12Rescale construct.");
}

nv12rescale::~nv12rescale()
{
	ALOGE("vaylb-->NV12Rescale destruct.");
}

void nv12rescale::nv12_nearest_scale(unsigned char* __restrict src, unsigned char* __restrict dst, int srcWidth, int srcHeight, int dstWidth, int dstHeight){
...
void nv12rescale::nv12_bilinear_scale (unsigned char* src, unsigned char* dst, int srcWidth, int srcHeight, int dstWidth,int dstHeight){
	int x, y;
    int ox, oy;
    int tmpx, tmpy;
    ...
    unsigned char* dst_uv = dst + dstHeight * dstWidth;
    unsigned char* src_y = src;
    unsigned char* src_uv = src + srcHeight * srcWidth;

    unsigned char y_plane_color[2][2];
    unsigned char u_plane_color[2][2];
    unsigned char v_plane_color[2][2];
    int j,i;
    ...
    for (i = 0; i < (dstWidth & ~7); ++i)
    {
        ...
        if (y_final<0)
            y_final = 0;
        dst_y[ j * dstWidth + i] = (unsigned char)y_final; //set Y in dest array 
            //UVUVUVUVUVUV
        if((j & 1) == 0) //j is even
        {
            if((i & 1) == 0) //i is even
       ...
       if((j & 1) == 0)
       {
           ...
           {    
               //set U in dest array  
               dst_uv[(j / 2) * dstWidth + i ] = (unsigned char)(u_sum / 4);
               //set V in dest array
               dst_uv[(j / 2) * dstWidth + i + 1] = (unsigned char)(v_sum / 4);
               u_sum = 0;
               v_sum = 0;
           }
       }
...
int nv12rescale::ImageResize(unsigned char * src, unsigned char* dst, int sw,int sh,int dw,int dh){
	if( (src == NULL) || (dst == NULL) || (0 == dw) || (0 == dh) || (0 == sw) || (0 == sh))
    {
        ALOGE("vaylb-->nv12rescale params error");
        return -1;
    }
